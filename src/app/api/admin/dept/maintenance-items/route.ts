...
export async function GET(request: NextRequest) {
  try {
    ...
    if (new URL(request.url).searchParams.get("stats") === "1") {
      const all = await prisma.adminMaintenanceItem.findMany({ where: { organizationId: guard.tenantId } });
    ...
    const data = await prisma.adminMaintenanceItem.findMany({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const record = await prisma.adminMaintenanceItem.create({
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updated = await prisma.adminMaintenanceItem.update({ where: { id }, data: { ...parsed.data, updatedAt: new Date() } });
