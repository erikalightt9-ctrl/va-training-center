...
import { voidInvoice } from "@/lib/repositories/acc-invoice.repository";
...
export async function POST(
  ...
  try {
    ...
    const result = await voidInvoice(tenantId, id, parsed.data.reason);
    return NextResponse.json({ success: true, data: result, error: null });
  } catch (err) {
    console.error("[POST /api/admin/accounting/invoices/[id]/void]", err);
    return NextResponse.json(
