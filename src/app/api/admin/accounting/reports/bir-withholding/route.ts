...
import { getBirWithholdingReport } from "@/lib/repositories/acc-bir.repository";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await getBirWithholdingReport(guard.tenantId, dateFrom, dateTo);
    return NextResponse.json({ success: true, data: result, error: null });
  } catch (err) {
    console.error("[GET /api/admin/accounting/reports/bir-withholding]", err);
