...
import {
  Wrench, Plus, Search, Loader2, AlertTriangle,
...
const EMPTY = { name: "", category: "", quantity: "", unit: "pcs", reorderLevel: "0", location: "", supplier: "", notes: "" };
...
export default function MaintenanceSuppliesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch(`/api/admin/office-admin/maintenance-supplies?${params}`);
      const json = await res.json();
      if (!json.success) throw new Error(json.error);
      setItems(json.data.items.map((i: Item & { quantity: number | string }) => ({ ...i, quantity: Number(i.quantity), reorderLevel: Number(i.reorderLevel) })));
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const body = { name: form.name, category: form.category || null, quantity: parseFloat(form.quantity) || 0, unit: form.unit, reorderLevel: parseFloat(form.reorderLevel) || 0, location: form.location || null, supplier: form.supplier || null, notes: form.notes || null };
      const url = editId ? `/api/admin/office-admin/maintenance-supplies?id=${editId}` : "/api/admin/office-admin/maintenance-supplies";
  ...
  const handleStockAction = async () => {
    ...
    try {
      ...
      const res  = await fetch(`/api/admin/office-admin/maintenance-supplies?id=${stockAction.id}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ quantity: stockAction.qty + delta }),
      });
      const json = await res.json();
      if (!json.success) throw new Error(json.error);
  ...
  const handleDelete = async (id: string) => {
    try {
      const res  = await fetch(`/api/admin/office-admin/maintenance-supplies?id=${id}`, { method: "DELETE" });
      const json = await res.json();
      if (!json.success) throw new Error(json.error);
  ...
  return (
    <div className="p-6 space-y-6">
      <nav className="flex items-center gap-1.5 text-xs text-slate-400">
        ...
        <ChevronRight className="h-3 w-3" /><span className="text-slate-700 font-medium">Maintenance Supplies</span>
      </nav>

      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-amber-600 flex items-center justify-center shrink-0"><Wrench className="h-5 w-5 text-white" /></div>
          <div>
            <h1 className="text-xl font-bold text-slate-900">Maintenance Supplies</h1>
            <p className="text-xs text-slate-400">Tools, cleaning materials, and repair consumables</p>
          </div>
        </div>
        <button onClick={() => { setShowForm((v) => !v); setEditId(null); setForm({ ...EMPTY }); }}
          className="flex items-center gap-2 px-3 py-2 bg-amber-600 text-white text-xs rounded-lg hover:bg-amber-700">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Total Items",  value: kpis.total,      icon: <Wrench className="h-4 w-4 text-amber-600" />,       cls: "" },
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          ...
          <form onSubmit={handleSave} className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div className="col-span-2 sm:col-span-3 flex gap-2 justify-end mt-1">
              ...
              <button type="submit" disabled={saving} className="flex items-center gap-1 px-3 py-1.5 text-xs bg-amber-600 text-white rounded-lg hover:bg-amber-700 disabled:opacity-50">
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search items..."
            className="w-full pl-8 pr-3 py-2 border border-slate-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-amber-500" />
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {["Item Name", "Category", "Stock", "Unit", "Min Level", "Status", "Location", "Supplier", "Actions"].map((h) => (
              ...
              <tbody className="divide-y divide-slate-100">
                {items.length === 0 ? (
                  <tr><td colSpan={9} className="px-4 py-12 text-center text-sm text-slate-400">No items found</td></tr>
                ) : items.map((item) => {
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => setStockAction({ id: item.id, name: item.name, qty, type: "issue" })} title="Issue" className="p-1.5 rounded-lg text-amber-600 hover:bg-amber-50"><ArrowUpFromLine className="h-3.5 w-3.5" /></button>
